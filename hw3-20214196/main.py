...
from simsiam.model_factory import Model
from simsiam.criterion import SSLLoss
...
parser.add_argument('--arch', default='resnet18', help='arch name is used for training')
parser.add_argument('--loss_type', default='contrastive', choices=['contrastive', 'align'])
parser.add_argument('--use_momentum_encoder', default='False', type=str2bool)
parser.add_argument('--m', default=0.99, help='momentum of the target-network')
parser.add_argument('--use_predictor', default='False', type=str2bool)
...
def main():
    ...
    if args.use_momentum_encoder:
        args.stop_gradient = True

    model_name = 'loss:' + args.loss_type + '_momentum:' + str(args.use_momentum_encoder) + \
                 '_predictor:' + str(args.use_predictor) + '_stop:' + str(args.stop_gradient)
    model_dir = path.join(args.exp_dir, model_name)
    ...
    model = Model(args)
    criterion = SSLLoss(args.loss_type, args.stop_gradient, args.use_predictor)
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if epoch % args.save_freq == 0:
            save_checkpoint(epoch, model, optimizer, val_top1_acc,
                            path.join(model_dir, 'ckpt_epoch_{}.pth'.format(epoch)),
                            'Saving...')
